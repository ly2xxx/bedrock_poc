...
def get_bedrock_llm():
    # Initialize the Bedrock LLM from langchain_aws
    # anthropic.claude-3-sonnet-20240229-v1
    # bedrock_llm = BedrockLLM(model_id='anthropic.claude-instant-v1')
    bedrock_llm = BedrockLLM(model_id='amazon.titan-text-express-v1')
    return bedrock_llm
    
def invoke_langchain_llm(bedrock_client,
                      messages,
    ...
    # Prepare the payload as a prompt
    prompt = json.dumps({
    ...
    # Invoke the model using the LLM instance
    response = llm(prompt)
    ...
    # match = re.search(r"<code>(.*)</code>", response, re.MULTILINE | re.DOTALL)
    # if match:
    #     return f'```py\n{match.group(1).strip()}\n```'
    # else:
    #     return "Sorry, I have not been able to answer your question. Would you mind trying again?"
...
def gen_program(csvfile, question, history=""):
    ...
    resp = invoke_langchain_llm(bedrock_client,messages)
    resp = resp[0]['text']
